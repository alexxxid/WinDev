...
class ApiAuteurController extends AbstractController
{
    ...
    public function create(Request $request, NationaliteRepository $repoNation, ObjectManager $manager, SerializerInterface $serializer, ValidatorInterface $validator)
    {
        ...
        $auteur->setNatinalite($nationalite);
        ...
        $manager->flush();
        return new JsonResponse("L'auteur à bien été crée", 201, [
     ...
     */
    public function update(Request $request, NationaliteRepository $repoNation, Auteur $auteur, ObjectManager $manager, SerializerInterface $serializer, ValidatorInterface $validator)
    {
        $data = $request->getContent();
        $dataTab = $serializer->decode($data, 'json');
        $nationalite = $repoNation->find($dataTab['nationalite']['id']);
